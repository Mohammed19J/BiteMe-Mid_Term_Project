...
import gui.SupplierHomePageController;
import gui.SupplierOrdersPageController;
import gui.UpdateMenuController;
...
public class ChatClient extends AbstractClient {
	...
	/**
	 * The current BranchReports object being used by the application.
	 * This object holds the branch reports data for the selected branch.
	 */
	public static BranchReports currBranchReports;
	/**
	 * The last month (as an integer) when the branch reports data was fetched.
	 * This is used to determine if the data needs to be refreshed based on the current month.
	 */
	public static int lastDataFetchMonth;
	/**
	 * The last month (as an integer) when the quarter reports data was fetched.
	 * This is used to determine if the quarter reports data needs to be refreshed based on the current month.
	 */
		...
		case SupplierRecievedNewOrder:
			...
			if (currController != null && currUser instanceof CertifiedWorker
				...
				if (currController instanceof SupplierOrdersPageController) {
					...
					Platform.runLater(() -> {
					((SupplierOrdersPageController) currController).showNotification("New order received\nOrder ID is: " + newRecievedOrderID,((SupplierOrdersPageController) currController).btnBack);
					});
				}
				else if (currController instanceof UpdateMenuController) {
					Platform.runLater(() -> {
					((UpdateMenuController) currController).showNotification("New order received\nOrder ID is: " + newRecievedOrderID,((UpdateMenuController) currController).btnBack);
					});
				}
				else if (currController instanceof SupplierHomePageController) {
					Platform.runLater(() -> {
					((SupplierHomePageController) currController).showNotification("New order received\nOrder ID is: " + newRecievedOrderID,((SupplierHomePageController) currController).lblHello);
					});
				}
			}
			break;
