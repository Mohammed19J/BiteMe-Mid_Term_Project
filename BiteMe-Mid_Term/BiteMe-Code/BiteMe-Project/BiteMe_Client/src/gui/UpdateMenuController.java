...
public class UpdateMenuController implements Initializable, IControl {
	...
	private boolean areFieldsFilled() {
		...
		if (!costField.getText().matches("\\d+(\\.\\d{1,2})?")) { // Allows integer or decimal values with up to 2
																	// decimal places
			showAlert("Cost Error", "Please ensure that the cost field contains only numbers.");
			return false;
		}
		switch (Category.valueOf(category)) {
		case Salad:
