...
public class CreateAccountController implements IControl, Initializable {
	...
	void getCreateAccountBtn(ActionEvent event) throws Exception {
		...
		User customer = new User(0, FNtxtfield.getText(), LNtxtfield.getText(), UNtxtfield.getText(),
				Passtxtfield.getText(), UserType.Customer, ChatClient.currUser.getBranchID(), Phonetxtfield.getText(),
				Emailtxtfield.getText(), CreditCardtxtfield.getText(), 0, 0, AccountTypeCB.getValue());
	...
	private boolean areAllFieldsFilled() {
	    ...
	    // Validate phone number: should contain only digits and be exactly 10 characters long
	    if (!Phonetxtfield.getText().matches("\\d{10}")) {
	        showAlert("Validation Error", "Phone number must contain exactly 10 digits.");
	        return false;
	    }

	    // Validate first name and last name: should contain only letters
	    if (!FNtxtfield.getText().matches("[a-zA-Z]+") || !LNtxtfield.getText().matches("[a-zA-Z]+")) {
	        showAlert("Validation Error", "First name and last name should contain only letters.");
	        return false;
		}

	    // Validate email: should contain an '@' symbol
	    ...
	    if (AccountTypeCB.getValue() == null || AccountTypeCB.getValue().isEmpty()) {
	        showAlert("Validation Error", "Please select an account type.");
	        return false;
	    }

	    // If account type is Private, validate credit card: should contain exactly 16 digits and no letters
	    if (AccountTypeCB.getValue().equals("Private")) {
	        if (!CreditCardtxtfield.getText().matches("\\d{16}")) {
	            showAlert("Validation Error", "Credit card number must contain exactly 16 digits.");
	            return false;
	        }
	    }
