...
public class ConnectToServerController implements IControl, Initializable {
	...
	void connectToServer(ActionEvent event) throws Exception {
		...
		if (ip.trim().isEmpty()) {
			showAlert("Enter IP","IP field cannot be empty.");
		} else if (!isValidIPAddress(ip)) {
			showAlert("IP Not Valid","Please enter a valid IP address.");
		} else if (!InetAddress.getByName(ip).isReachable(1000)) {
			showAlert("IP Not Valid","The IP you entered is unreachable, Please enter a valid IP address.");
		} else {
